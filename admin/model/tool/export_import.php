...
class ModelToolExportImport extends Model {
	...
	protected function validateOptionColumns( &$reader ) {
		...
		for ($i=1; $i<$k; $i+=1) {
			...
			if ($product_id=="") {
				if (!$has_missing_product_ids) {
					$msg = str_replace( '%1', 'ProductOptions', $this->language->get( 'error_missing_product_id' ) );
					$this->log->write( $msg );
		...
		for ($i=1; $i<$k; $i+=1) {
			...
			if ($product_id=="") {
				if (!$has_missing_product_ids) {
					$msg = str_replace( '%1', 'ProductOptionValues', $this->language->get( 'error_missing_product_id' ) );
					$this->log->write( $msg );
					$has_missing_product_ids = true;
				}
				$ok = false;
	...
	protected function validateAttributeColumns( &$reader ) {
		...
		for ($i=1; $i<$k; $i+=1) {
			...
			if ($product_id=="") {
				if (!$has_missing_product_ids) {
					$msg = str_replace( '%1', 'ProductAttributes', $this->language->get( 'error_missing_product_id' ) );
					$this->log->write( $msg );
					$has_missing_product_ids = true;
				}
				$ok = false;
...
//			$worksheet->getStyle( $range )->applyFromArray( $style, false );
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("http://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_2_21&language_code=$language_code");
